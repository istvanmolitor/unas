...
class UnasCopyProductCategories extends Command
{
    private const DIRECTION_MASTER_TO_SHOP = 'master-to-shop';

    private const DIRECTION_SHOP_TO_MASTER = 'shop-to-master';

    protected $signature = 'unas:copy-product-categories {--shop-id=} {--direction=}';

    protected $description = 'Copy product categories between product master and a selected UNAS shop';
    ...
    private function copyMasterToShop(UnasShop $shop): int
    {
        ...
        foreach ($this->productCategoryRepository->getAll() as $productCategory) {
            $path = $this->productCategoryRepository->getPath($productCategory);
            $this->unasProductCategoryService->createByPath($shop, $path);
            $copiedCategoriesCount++;
        }

        $this->info("Copied {$copiedCategoriesCount} product categor(ies) from product master to shop: {$shop->name}");
    ...
    private function copyShopToMaster(UnasShop $shop): int
    {
        ...
        /** @var UnasProductCategory $unasProductCategory */
        foreach ($this->unasProductCategoryRepository->getByShop($shop) as $unasProductCategory) {
            $path = $this->unasProductCategoryService->getPath($unasProductCategory);
            $this->productCategoryRepository->createProductCategory($path);
            $copiedCategoriesCount++;
        }

        $this->info("Copied {$copiedCategoriesCount} product categor(ies) from shop {$shop->name} to product master");
